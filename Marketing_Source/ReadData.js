...
$.getJSON(url, function (json) {
  ...
  for (var i = 0; i < json.records.length; i++) {
    //alert(typeof json.records[i].SalesReport);
      var dating = new Date(json.records[i].Date_Start);
      var today = new Date();
...
function createHeader(){
  ...
  document.getElementById('Table-Source').style.display = "block";
...
function provideDetails(){
  ...
  $.getJSON(url, function (json) {
    ...
    evento.forEach((item, i) => {
      if(opt.value == item){
        document.getElementById('date').innerHTML = "Date: "+new Date(json.records[i].Date_Start).toDateString();
        document.getElementById('time').innerHTML = "Time: "+ new Date(json.records[i].Time).toLocaleTimeString();
        document.getElementById('Venue').innerHTML = "Venue: "+ json.records[i].Host_Venue;
